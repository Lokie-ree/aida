...
// Context for coach actions
const CoachActionsContext = createContext<{
  onCoachAction?: (action: 'newSession' | 'viewLibrary') => void;
  setCoachActionHandler?: (handler: (action: 'newSession' | 'viewLibrary') => void) => void;
}>({});

export const useCoachActions = () => useContext(CoachActionsContext);

// Component to handle authenticated header with navigation
function AuthenticatedHeader() {
  const { data: session } = authClient.useSession();
  const { onCoachAction } = useCoachActions();
...
export default function App() {
  ...
  const [coachActionHandler, setCoachActionHandler] = useState<
    ((action: 'newSession' | 'viewLibrary') => void) | undefined
  >();
      ...
      <ThemeProvider>
        <BrowserRouter>
          <CoachActionsContext.Provider
            value={{
              onCoachAction: coachActionHandler,
              setCoachActionHandler: (handler) => setCoachActionHandler(() => handler),
            }}
          >
